      ...
      <x-modal name="edit-produk">
       <div class="flex justify-center items-center">
        <div class="px-8 py-6 max-w-md">
         ...
         <form id="formEditProduk" method="POST" action="{{ route('dashboard.produk.edit', $produk->produk_id) }}">
          @csrf

          {{-- SKU Produk --}}
          <div class="mb-4">
           <x-input-label for="sku" value="{{ __('SKU Produk') }}" />
           <x-text-input id="sku" name="sku" type="text" class="mt-1 block w-full" placeholder="{{ __('SKU Produk') }}"
            required />
          </div>
